...
<div id="modal-posttypes" data-modal-backdrop="" tabindex="-1" aria-hidden="true"
	class="hidden modal-wrapper justify-center items-center">
	<!-- Modal container -->
	<div class="modal-container">
		...
		<div class="modal-inner">
						...
						<li class="list-item" id="type-<?php echo $slug; ?>">
							<div class="list-item-body">
								...
								<label class="inline-flex items-center cursor-pointer ml-auto relative">
									<input type="checkbox" value="" class="sr-only peer cacheable-post-type"
										name="<?php echo $slug; ?>" id="post-type-post-status" <?php if ( $npCPT['isOptimized'] )
											   echo 'checked'; ?>>
								...
								<?php foreach ( $npCPT['taxonomies'] as $tax_slug => $taxonomyType ) { ?>
									<ul class="sub-menu">
										<div class="list-item-body" id="tax-<?php echo $tax_slug; ?>">
											...
											<label class="inline-flex items-center cursor-pointer ml-auto relative">
												<input type="checkbox" class="sr-only peer cacheable-post-type"
													name="<?php echo $tax_slug; ?>" id="post-type-post-status" <?php if ( $taxonomyType['isOptimized'] )
														   echo 'checked'; ?>>
												<div class="toggle"></div>
											</label>
			...
			<div class="modal-footer">
				<button data-modal-hide="modal-posttypes" type="button" class="btn btn-secondary">
					<?php esc_html_e( 'Close', 'nitropack' ); ?>
				</button>
				<button data-modal-hide="modal-posttypes" type="button" class="btn btn-primary"
					id="save-cacheable-post-types">
					<?php esc_html_e( 'Save Changes', 'nitropack' ); ?>
				</button>
			</div>
		</div>
	</div>
